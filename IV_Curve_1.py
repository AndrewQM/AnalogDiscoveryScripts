...
ptspp = 20 #int(input("Number of datapoints per period? "))
...
while cSamples < nSamples:
    ...
    #progress bar made of # signs

    #if (float(cSamples)/float(nSamples)) > (float(display+1)/float(50)):
...
for i in range(begin,len(rgpy)):
    rgpy[i]=rgdSamples[i]
    rgpz[i]=rgdSamples2[i]

plt.plot(rgpy, color = '#962424') #rg or grpy is direct waveform generator voltage
plt.plot(rgpz, color = '#249096')
plt.show()
